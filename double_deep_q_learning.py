...
class DDQNAgent:
    ...
    def __init__(self,
        ...
        self._log_dir = "logs/fit/" + "DDQN" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def _sample_action_and_choose_model(self, state):
        ...
        if random.uniform(0, 1) > 0.5:
            value_model = self.model_1
            target_model = self.model_2
        else:
            value_model = self.model_2
            target_model = self.model_1

        if random.uniform(0, 1) < self.epsilon:
            action = self.env.action_space.sample()
        else:
            q_values = value_model.predict(state)[0]
            action = np.argmax(q_values)

        return action, value_model, target_model
    ...
    def _replay(self, batch_size, total_step_number, step, value_model, target_model):
        """
        ...
        for state, action, reward, next_state, done in minibatch:
            if done and step < 500:  # for terminal transition
                ...
                target = (reward + self.gamma * np.max(target_model.predict(next_state)[0]))

            # update y
            target_f = value_model.predict(state)
        ...
        fit_result = value_model.fit(np.array(states_in_batch),
    ...
    def train_agent(self,
        ...
        for i in tqdm(range(1, episodes+1)):
            ...
            for step in range(1, steps_per_episode + 1):
                ...
                # sample random minibatch, update y, and perform gradient descent step
                self._replay(batch_size, total_steps, step, value_model, target_model)
    ...
    def test_agent(self, episodes):
        ...
        for _ in range(episodes):
            ...
            while not done:
                ...
                action = np.argmax(self.model_1.predict(state)[0])
